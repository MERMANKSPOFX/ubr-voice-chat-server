...
app.post('/api/voice', (req, res) => {
  const { playerName, serverAddress, audioData, audioSize, sampleRate, channels } = req.body;
  
  if(!playerName || !serverAddress || !audioData) {
    return res.status(400).json({ error: 'Missing playerName, serverAddress or audioData' });
  }
  
  // Ensure player is registered for this server
  if(!players.has(serverAddress)) {
    players.set(serverAddress, new Set());
  }
  if(!players.get(serverAddress).has(playerName)) {
    players.get(serverAddress).add(playerName);
    console.log(`Registered player ${playerName} for server ${serverAddress}`);
  }
  
  // Broadcast audio to all players on the same server except sender
  const playersOnServer = players.get(serverAddress);
  
  let sentCount = 0;
  for(const [name, socket] of voiceConnections.entries()) {
    // Only send to players on the same server and not to sender
